...
- Pixiv 直连分支调整为混合链路：先保留 Accesser 风格域名解析覆盖，再在同一候选失败时回退到 PixEz 风格禁用 TLS SNI + 跳过证书校验
- 新增 `disable_bypass_sni` 运行时配置，关闭后会跳过 DoH 刷新并直接使用普通域名请求

### v1.10.8

- 搜索接口在运行时 DNS 模式下若所有候选 IP 都返回 `403`，现在会再补一次原域名直连
- 降低 `search_illust` / `search_user` 因固定 IP 与别名 IP 全部被拒绝而直接失败的概率

### v1.10.7

- `/pixiv search` 在没有作品结果时会自动切换为作者搜索，并返回匹配到的作者
- 仅在作品搜索为空时触发，不影响已有作品搜索命中时的原有行为

### v1.10.6

- 修复 Pixiv 搜索接口布尔参数仍按 Python `True/False` 发送的问题，避免重试后返回 `400 invalid value`
- `/pixiv search` 现在会把相关开关参数规范化为 Pixiv 接口接受的 `"true"` / `"false"` 字符串

### v1.10.5

- 修复 `/pixiv search` 与 `/pixiv searchuser` 在运行时 DNS 重试时，命中返回 `403` 的旧 IP 后不会继续尝试后续候选 IP 的问题
- 搜索接口现在会在 IP 直连分支继续试探其他动态解析 IP，减少持续报错 `Pixiv API 请求失败（状态码=403）`
...
- **DNS 优化**：支持 Accesser + PixEz 混合直连模式，并可按配置关闭
...
```
查看 DNS 刷新状态或手动触发刷新。默认启用混合直连模式：优先使用 Accesser 风格域名解析覆盖，请求失败时再回退到 PixEz 风格禁用 SNI 的 IP 直连；若设置 `disable_bypass_sni=true`，则会跳过 DoH 刷新并直接使用普通域名请求。
