...
- 重写 `/pixiv help` 帮助菜单，改为分组展示基础、搜索、随机收藏、设置和管理命令
- 补充多个常见子命令的定向用法提示
...
- 优化 `commands.py` 中随机缓存补货与缓存取回逻辑，统一共享随机与自用随机的补货调用路径

### v1.8.6

- 优化 `commands.py` 中共享目标解析与 `warmup` 参数解析逻辑
- 统一 QQ `At` 组件和 `@用户名` 文本两条共享解析路径

### v1.8.5

- 优化 `commands.py` 中随机收藏结果发送路径，统一缓存命中/新获取、共享/自身场景下的消息构造逻辑
- 统一剩余缓存统计与 R-18 文本回退处理

### v1.8.4

- 优化 `image_handler.py`，统一图片与动图 zip 下载逻辑，并复用动图帧元数据处理代码
- 优化 `emoji_reaction.py`，抽取 aiocqhttp 消息上下文与阶段表情解析逻辑
...
- 将群聊 R-18 自动打码替换为内置的真实 AutoHajimiMosaic 哈基米打码流程
- 仅保留打码所需模型与素材，移除仓库内无关的 Web/UI、Docker 与脚本文件

### v1.8.0

- 新增群聊内 R-18 标签显示开关：`/pixiv random r18 tag true/false`
- 新增群聊内 R-18 图片自动打码开关：`/pixiv random r18 mosaic true/false`
- 群聊发送 R-18 图片时可按配置自动生成打码版本

### v1.7.3

- 修复 `/pixiv dns refresh` 在当天已刷新过 DNS 时仍可能不生效的问题
- 修复 `/pixiv random cache now N` 会把旧缓存误算为本次成功数量的问题
- 修复闲时缓存与即时缓存成功后未持久化最新 refresh token 的问题
- 修复 `/pixiv searchuser` 复用插画搜索选项解析导致透传不适用参数的问题

### v1.7.2

- 修复搜索时偶发返回 440、429 或 5xx 后直接失败的问题
- 搜索失败后会自动刷新 DNS 并重试一次

### v1.7.1

- 修复 `/pixiv dns refresh` 未真正触发刷新
- 修复 `count=` 与 `random=true` 参数未生效
- 修复唯一随机模式未排除已发送作品
- 支持多词搜索关键词
- 补齐 `/pixiv groupblock`、`/pixiv config` 等顶级命令别名
...
```
查看 DNS 刷新状态或手动触发刷新。插件会在每天凌晨 4 点自动刷新 DNS 缓存。
...
### v1.6.0
- **唯一模式优化**：记录已发送图片ID，缓存时自动避开，支持扩展扫描范围和彻底随机模式
- **多图片支持**：多图片作品自动处理，<=3张直接发送，>3张使用合并转发
- **图片质量设置**：支持按用户/群组设置图片质量（原图/中等/小图）
- **配置管理**：新增 `/pixiv config` 命令管理插件内部常量
- **DNS 状态显示**：`/pixiv dns` 命令现在显示下次刷新时间
- **修复缓存 count**：修复闲时缓存 count 参数无效的问题
- **常量值修正**：修正 `IDLE_CACHE_INTERVAL_SECONDS` 为 900 秒，`IDLE_CACHE_COUNT` 为 5
...
### v0.3.0
- **统一缓存池**：所有缓存图片存储在统一池中，支持跨筛选条件复用
- **元数据筛选**：缓存时记录完整元数据（标签、作者、R-18 标识），筛选时精准匹配
- **R-18 群聊过滤**：群聊中 R-18 内容仅发送详情，不发送图片（管理员可通过 `/pixiv random r18 true/false` 配置）
- **illustID 缓存优化**：通过 `/pixiv id i` 获取的图片自动加入随机缓存池，重复请求直接从缓存发送
- **免 token 操作**：`/pixiv random share`、`/pixiv random r18`、`/pixiv random @用户名` 均不再需要当前用户绑定 token
- **ffmpeg 回退**：动图渲染 PIL 失败时自动回退 ffmpeg
- **修复 Share 配置**：修复 share 配置命令被 token 检查阻断的问题
