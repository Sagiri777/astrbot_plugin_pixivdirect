...
- 调整 PixEz 模式的 SNI 绕过策略：App API 现在保留域名 TLS/SNI，仅覆盖 DNS 到缓存 IP；图片链路继续走固定 IP + 禁用 SNI
...
- 新增 `scripts/test_bypass_modes.py`，用于按保守节流频率测试 `auto`、`pixez`、`accesser` 与直连模式下的请求表现
...
- **DNS 优化**：支持 PixEz 优先的 DoH + 缓存 IP 直连模式，App API 默认保留域名 SNI，图片链路继续禁用 SNI，并可按配置关闭
...
```
查看 DNS 刷新状态或手动触发刷新。默认启用 PixEz 优先模式：插件启动时立即执行 DoH 解析，并在每天凌晨 4 点重跑一次；后续请求会优先尝试 PixEz 直连链路，其中 App API 保留域名 TLS/SNI 并仅覆盖 DNS 到缓存 IP，图片请求继续使用固定 IP + 禁用 SNI；若该候选失败再回退到 Accesser 风格覆盖。若设置 `disable_bypass_sni=true`，则会跳过 DoH 刷新并直接使用普通域名请求。
...
- `pixez`：App API 保留域名 SNI 并覆盖到缓存 IP，图片继续走固定 IP + 禁用 SNI 的直连链路。
