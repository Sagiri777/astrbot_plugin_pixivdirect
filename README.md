...
- 补充回归测试，覆盖 PixEz 风格的 OAuth、App API、图片请求、Web 搜索和 `illust_recommended` 参数行为
...
- 网络层按本地 pixez-flutter 重新对齐：App API、OAuth 与图片请求默认走 PixEz 风格 DNS 覆盖、禁用 SNI 与跳过证书校验
- `auto` / `accesser` 不再作为公开可切换模式，Accessor 仅保留为内部故障恢复备用链路

### v1.12.0

- 新增“收藏元数据缓存 + 新用户两天预热”链路，`/pixiv random` 现在默认按“本地图片缓存 > 元数据缓存 > 实时随机”取图
- 新增 `/pixiv random source` 与 `/pixiv imagehost`，支持切换读取策略并接入通用 HTTP 图床

### v1.11.8

- 修复 `scripts/test_bypass_modes.py` 在初始化或单项检查抛异常时会直接退出的问题，改为继续执行并输出最终结果表
...
- **元数据预热**：新用户首次登录后，会在 2 天内后台分批扫描公开收藏并缓存元数据
- **空闲缓存**：程序空闲时自动为所有用户预缓存随机图片，并优先预热最近 7 天最高频的 random 筛选条件
- **随机三级回退**：`/pixiv random` 默认优先命中本地图片缓存，其次命中已缓存元数据，最后才调用实时随机接口
- **通用图床接入**：支持将本地图片上传到任意支持 `multipart/form-data` 的 HTTP 图床接口，并在消息中附上图床链接
...
- **PixEz 主模式**：默认按本地 PixEz 请求方式访问 Pixiv，App API、OAuth 与图片请求统一走 DNS 覆盖 + 禁用 SNI
- **DNS 优化**：启动时默认优先刷新图片 host 缓存，管理员可手动刷新全部 PixEz host；可通过 `disable_bypass_sni` 一键关闭
...
- [main.py](/Users/guozimier/Downloads/AstrBot/data/plugins/astrbot_plugin_pixivdirect/main.py)：插件主类、生命周期、后台任务与命令入口
- [plugin/entry.py](/Users/guozimier/Downloads/AstrBot/data/plugins/astrbot_plugin_pixivdirect/plugin/entry.py)：稳定导出入口
- [commands.py](/Users/guozimier/Downloads/AstrBot/data/plugins/astrbot_plugin_pixivdirect/commands.py)：现有命令编排层
- [infrastructure/pixiv_client.py](/Users/guozimier/Downloads/AstrBot/data/plugins/astrbot_plugin_pixivdirect/infrastructure/pixiv_client.py)：内建 Pixiv 客户端，按本地 PixEz 行为实现 OAuth、App API、图片下载和 DNS/SNI 逻辑
- [domain/models.py](/Users/guozimier/Downloads/AstrBot/data/plugins/astrbot_plugin_pixivdirect/domain/models.py) 与 [domain/errors.py](/Users/guozimier/Downloads/AstrBot/data/plugins/astrbot_plugin_pixivdirect/domain/errors.py)：领域模型与错误定义

> `pixivSDK.py` 目前仅保留为历史迁移对照文件，插件运行时不再直接调用它。
...
```
/pixiv random source
/pixiv random source metadata
/pixiv random source image
```

- `metadata`：默认推荐模式。执行顺序为“本地图片缓存 > 元数据缓存 > 实时随机”。
- `image`：只走原有本地图片缓存与实时随机链路，不使用元数据层补图。
- 无论哪种模式，只要最终准备发送某个作品，都会先按 `illust_id` 检查本地缓存是否已有图片。

#### 10.2 新用户元数据预热

- 用户首次 `/pixiv login` 成功后，会自动创建一个 2 天的预热窗口。
- 预热任务只扫描该用户的**公开收藏**，不会扫描私密收藏。
- 后台会分页抓取收藏页面中的作品元数据，并写入 `bookmark_metadata_cache.json`。
- 元数据缓存默认保留 48 小时，过期条目会在加载或写入时自动清理。

#### 10.3 图片缓存模式与元数据模式差异

- 图片缓存命中最快，适合已经下载过的作品。
- 元数据缓存适合“作品信息已知但图片尚未落地”的场景，命中后会即时下载图片并自动回填本地图片缓存。
- 实时随机仍保留为最后兜底，因此首次使用或筛选极冷门条件时依然可正常工作。
- 共享收藏时也遵循相同优先级：先看本地图片缓存，再看对方已缓存元数据，最后再走实时随机。
...
```
查看 DNS 刷新状态或手动触发刷新。默认启用 PixEz 主模式：插件启动时优先刷新 `i.pximg.net` / `s.pximg.net` 的 DoH 缓存，并在每天凌晨 4 点重跑一次；App API、OAuth 与图片请求默认按 PixEz 风格使用缓存 IP、禁用 SNI 并跳过证书校验。若设置 `disable_bypass_sni=true`，则会跳过 DoH 刷新并直接使用普通域名请求。
...
- `pixez`：唯一公开模式，默认按本地 PixEz 的请求方式访问 Pixiv。
- `auto` / `accesser`：已废弃，不再作为公开可切换模式；Accessor 仅在内部故障恢复时备用。
- 若 `disable_bypass_sni=true`，则会直接走普通域名请求。
...
/pixiv imagehost enable true
/pixiv imagehost set endpoint https://img.example.com/api/upload
/pixiv imagehost set method post
/pixiv imagehost set file_field file
/pixiv imagehost set success_path data.url
/pixiv imagehost header set Authorization Bearer your-token
/pixiv imagehost field set album pixiv
```

- 图床启用后，插件会在发送首图前尝试把本地图片上传到外部图床。
- 第一版以兼容性为先：即使上传成功，也仍保留本地文件发送，同时在文本中附上图床链接。
- 若上传失败、响应非 JSON，或 `success_path` 没取到 URL，会自动回退到原本地发送链路。

**通用 HTTP 图床接入教程**

1. 准备一个支持 `multipart/form-data` 上传的接口，并确认：
   - 上传地址 `endpoint`
   - 上传方法 `POST` 或 `PUT`
   - 文件字段名 `file_field`
   - 返回 JSON 中图片 URL 的路径 `success_path`
2. 以一个返回如下 JSON 的自建接口为例：

```json
{
  "success": true,
  "data": {
    "url": "https://img.example.com/uploads/demo.jpg"
  }
}
```
...
/pixiv imagehost set file_field file
/pixiv imagehost set success_path data.url
/pixiv imagehost header set Authorization Bearer your-demo-token
/pixiv imagehost field set album pixivdirect
/pixiv imagehost enable true
```

4. 用 `/pixiv imagehost status` 检查配置是否生效。

**图床排查**

- `success_path` 错误：上传成功但消息里没有图床链接，检查 JSON 层级是否和实际响应一致。
- 鉴权 header 错误：接口返回 `401/403`，检查 `header set` 的键和值是否正确。
- 返回非 JSON：当前实现会直接回退本地发送，需确认服务端是否返回 JSON。
- 上传成功但仍走本地发送：这是第一版的预期兼容策略，插件会附带图床链接，但不强制改成远程 URL 发图。
...
2. **infrastructure/pixiv_client.py**：内建 Pixiv 客户端
   - OAuth 认证流程
   - App API / Web 搜索请求封装
   - 图片与动图下载
   - PixEz 风格 DNS / SNI 处理
