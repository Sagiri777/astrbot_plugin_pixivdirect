...
- 为图片下载、缓存写入、缓存命中、发送前判定和动图渲染补充更细粒度日志
- 更容易定位图片获取、保存或发送链路卡住的位置

### v1.10.3

- `/pixiv search` 与 `/pixiv searchuser` 现会在 Pixiv 返回 403 时自动刷新 DNS，并启用运行时解析后重试
- 降低搜索请求命中失效固定 IP 后持续报错 `Pixiv API 请求失败（状态码=403）` 的概率

### v1.10.2

- `/pixiv random` 支持同一字段内多个正负筛选条件混用（如 `tag=风景&!R-18`、`author=画师A&！画师B`）
- 修复多条件筛选时缓存匹配不完整的问题，并补充多条件筛选示例

### v1.10.0

- 新增最近 7 天 random 筛选条件使用统计，包含用户自己使用和别人通过共享访问其收藏的筛选行为
- 闲时缓存在无显式任务队列时，会优先预热该用户最近 7 天最常用的筛选条件
