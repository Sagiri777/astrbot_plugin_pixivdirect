...
- **关闭时（默认）**：群聊中随机到 R-18 图片时，仅发送作品详情（ID、标题、标签等），不发送图片
- **开启时**：群聊中正常发送 R-18 图片
...
|------|-----------|
| 私聊随机 | 正常发送图片 |
| 群聊随机（r18=false） | 仅发送详情，不发图片 |
| 群聊随机（r18=true, mosaic=false） | 发送原图 |
| 群聊随机（r18=true, mosaic=true） | 发送自动打码图 |
| `/pixiv id i` 指定获取 | 始终发送图片 |

判断依据：`x_restrict >= 1` 或标签包含 `R-18`/`R18`/`R-18G`。当群聊内关闭 R-18 标签显示时，会隐藏消息中的标签行，但保留其他说明文字。
