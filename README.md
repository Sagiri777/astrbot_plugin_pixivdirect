...
- 新增 `/pixiv bypass`，可在 `auto`、`pixez`、`accesser` 三种绕过模式之间切换
- 搜索命令新增 App API 重试、Pixiv Web 搜索兜底、代理兜底和 3 天粘滞代理窗口

### v1.10.11

- 修复 `aiocqhttp` 发送超大本地缓存静态图时容易在 QQ 侧超时的问题
- 发送前会自动复用压缩后的发送缓存，限制尺寸和体积，降低随机收藏与作品图片发送失败概率

### v1.10.10

- DNS 刷新流程改为更贴近 PixEz：插件启动时立即执行 DoH 解析，并在每天凌晨 4 点后台重跑一次
- 请求阶段默认优先使用缓存 IP + 禁用 TLS SNI；仅在该候选失败时才回退到 Accesser 风格的域名请求覆盖

### v1.10.9

- Pixiv 直连链路调整为混合模式：优先保留 Accesser 风格的域名解析覆盖，并在同一候选失败后回退到 PixEz 风格的禁用 TLS SNI + 跳过证书校验
- 新增 `disable_bypass_sni` 运行时配置，可按需关闭 DoH + 禁用 SNI 直连
...
- **搜索恢复**：`/pixiv search` / `/pixiv searchuser` 支持 App API 重试、Pixiv Web 搜索兜底和代理兜底
- **绕过模式切换**：支持 `auto`、`pixez`、`accesser` 三种绕过模式，并兼容 `disable_bypass_sni`
- **DNS 优化**：支持 PixEz 优先的 DoH + 缓存 IP + 禁用 SNI 直连模式，并可按配置关闭
- **QQ 发送优化**：`aiocqhttp` 发送前会自动为超大静态图生成更轻量的发送缓存，降低上传超时概率
